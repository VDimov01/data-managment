...
router.post('/vehicles/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...out,
      qr_object_key: out.qr_object_key ?? out.objectKey,
      qr_png_path: out.qr_png_path ?? out.publicUrl,
    });
  } catch (e) {
...
router.get('/vehicles/:id/qr.png', async (req, res) => {
  ...
  const [[row]] = await getPool().query(
    'SELECT public_uuid, qr_object_key FROM vehicle WHERE vehicle_id=?', [id]
  );
