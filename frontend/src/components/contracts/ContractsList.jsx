      ...
      <div className="card-body">
        <div className="row" style={{ alignItems: 'flex-end' }}>
          <div className="col">
            ...
            <input className="inp" placeholder="Номер, клиент, UUID…" value={q} onChange={e=>setQ(e.target.value)} />
          </div>
        ...
        {pages > 1 && (
          <div className="pager">
            <button className="btn" disabled={page<=1} onClick={()=>setPage(p=>Math.max(1,p-1))}>Предишна</button>
            <span>Страница {page} / {pages}</span>
            <button className="btn" disabled={page>=pages} onClick={()=>setPage(p=>Math.min(pages,p+1))}>Следваща</button>
          </div>
        )}
        {attachmentsFor && (
        <AttachmentsModal
